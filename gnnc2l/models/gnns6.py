...
# PyroGCN
class PyroGCNModel_6Layer(PyroModule):
    ...
    def __init__(self, input_dim=11, output_dim=1, emb_dim=64, adj=None, activation_fn=None, bias=True):
        super(PyroGCNModel_6Layer, self).__init__()
        ...
        self.linear5 = PyroModule[nn.Linear](emb_dim, emb_dim)
        self.linear6 = PyroModule[nn.Linear](emb_dim, emb_dim)
...
class GATAModel_6Layer(PyroModule):
    ...
    def __init__(self, input_dim, output_dim, emb_dim=64, adj=None, edge_dim=None, activation_fn=None, bias=True):
        super(GATAModel_6Layer, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.emb_dim = emb_dim
        self.adj = adj
        ...
        # 5th Layer
        self.conv5 = PyroModule[GATv2Conv](in_channels=4*emb_dim, out_channels=emb_dim, heads=4, concat=True, dropout=0.2, add_self_loops=True, edge_dim=edge_dim, bias=bias)

        # 6th Layer
        self.conv6 = PyroModule[GATv2Conv](in_channels=4*emb_dim, out_channels=emb_dim, heads=4, concat=True, dropout=0.2, add_self_loops=True, edge_dim=edge_dim, bias=bias)
    ...
    def forward(self, data):
        ...
        x, attention_weights = self.conv5(x, data.edge_index, return_attention_weights=True)
        if self.activation_fn is not None:
            x = self.activation_fn(x)

        x, attention_weights = self.conv6(x, data.edge_index, return_attention_weights=True)
        if self.activation_fn is not None:
            x = self.activation_fn(x)
