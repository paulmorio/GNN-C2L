...
from gnnc2l.models.gnns2 import *
from gnnc2l.models.gnns3 import *
from gnnc2l.models.gnns4 import *
from gnnc2l.models.gnns5 import *
from gnnc2l.models.gnns6 import *
...
class DirectGNNLocationModelLinearDependentWMultiExperimentLocationBackgroundNormLevelGeneAlphaPyroModel(PyroModule):
    r"""
    ...
    def __init__(
        ...
        n_batch,
        cell_state_mat,
        # adj,  # Adjacency matrix for GNN. TODO: Remove
        edge_index,  # Adjacency matrix for GNN.
        edge_attr,  # Adjacency matrix for GNN.
        pos, # Spot positions
        gnn_type = 'InvariantMPNN',
        n_groups: int = 50,
        detection_mean=1 / 2,
        ...
        **gnn_kwargs
        ...
        # Instantiate GNN
        ####################
        print('input_dim', self.n_vars)
        print('output_dim', self.n_factors)
        
        self.edge_index = edge_index  # Adjacency matrix for GNN.
        self.edge_attr = edge_attr# Adjacency matrix for GNN.
        self.pos = pos # Spot positions
        
        # TODO: All GNNs in same format
        print('GNN type: ', gnn_type)
        if gnn_type == 'GCNModel':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            # print('Adj shape: ', adj.shape)
            self.gnn = GCNModel(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'PyroGCNModel':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
            print("Using PyroGCN!")
        elif gnn_type == 'PyroMLP':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroMLP(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
            print("Using PyroMLP!")
        elif gnn_type == 'GATAModel':
            self.gnn = GATAModel(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())
            print("Using GATAModel!")
        elif gnn_type == 'PyGPyroGCNModel':
            self.gnn = PyGPyroGCNModel(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())
            print("Using PyG version of PyroGCNModel!")
        elif gnn_type == 'InvariantMPNN':
            # self.gnn = InvariantMPNNModel(in_dim=self.n_factors, out_dim=self.n_factors, activation_fn=torch.nn.ReLU(), edge_dim=edge_attr.shape[1], **gnn_kwargs)
            self.gnn = InvariantASFModel(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())
        elif gnn_type == 'EquivariantMPNN':
            # self.gnn = EquivariantMPNNModel(in_dim=self.n_factors, out_dim=self.n_factors, activation_fn=torch.nn.ReLU(), edge_dim=edge_attr.shape[1], **gnn_kwargs)
            self.gnn = EquivariantASFModel(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())

        # 2 Layer variants
        elif gnn_type == 'PyroGCNModel_2Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel_2Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'PyroMLP_2Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroMLP_2Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'GATAModel_2Layer':
            self.gnn = GATAModel_2Layer(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'InvariantMPNN_2Layer':
            self.gnn = InvariantASFModel_2Layer(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())
        elif gnn_type == 'EquivariantMPNN_2Layer':
            self.gnn = EquivariantASFModel_2Layer(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())

        # 3 Layer variants
        elif gnn_type == 'PyroGCNModel_3Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel_3Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'PyroMLP_3Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroMLP_3Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'GATAModel_3Layer':
            self.gnn = GATAModel_3Layer(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'InvariantMPNN_3Layer':
            self.gnn = InvariantASFModel_3Layer(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())
        elif gnn_type == 'EquivariantMPNN_3Layer':
            self.gnn = EquivariantASFModel_3Layer(input_dim = self.n_factors, output_dim=self.n_factors, edge_dim=edge_attr.shape[1], activation_fn=torch.nn.Softplus())

        # 4 Layer variants
        elif gnn_type == 'PyroGCNModel_4Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel_4Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'GATAModel_4Layer':
            self.gnn = GATAModel_4Layer(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())


        # 5 Layer variants
        elif gnn_type == 'PyroGCNModel_5Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel_5Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'GATAModel_5Layer':
            self.gnn = GATAModel_5Layer(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())


        # 6 Layer variants
        elif gnn_type == 'PyroGCNModel_6Layer':
            adj = torch_geometric.utils.to_dense_adj(edge_index)[0]  # Function to_dense_adj returns single-batched dense adj
            self.gnn = PyroGCNModel_5Layer(input_dim=self.n_factors, output_dim=self.n_factors, adj=adj, activation_fn=torch.nn.Softplus())
        elif gnn_type == 'GATAModel_6Layer':
            self.gnn = GATAModel_5Layer(input_dim = self.n_factors, output_dim = self.n_factors, activation_fn=torch.nn.Softplus())
    ...
    def forward(self, x_data, idx, batch_index):
        # batch_index: Shape=(# spots, 1)
        # idx: Shape=(# spots,)
        # x_data: Shape=(# spots, # genes)
        ...
        # Prepare data object for GNN
        # data = Data(x=x_data, pos=self.pos, edge_index=self.edge_index, edge_attr=self.edge_attr)
        
        # # Register module so that Pyro knows about all the parameters the gnn. Ref: https://pyro.ai/examples/vae.html
        if self.gnn is not None:
            pyro.module("gnn", self.gnn)

        with obs_plate as ind:
            w_sf_mu = z_sr_groups_factors @ x_fr_group2fact   # w_sf_mu. Shape=(# spots, # CTs)
            # print('w_sf shape: ', w_sf_mu.shape)
            # print('z_sr_groups_factors', z_sr_groups_factors.shape)
            # print('x_fr_group2fact', x_fr_group2fact.shape)
            # print('Using GNN to compute w_sf_mu')
            ...
            ################# Creating a_sf
            #################################################
            data = Data(x=w_sf, pos=self.pos, edge_index=self.edge_index, edge_attr=self.edge_attr)
            k = "a_sf"
            a_sf = pyro.sample(
                k,
                dist.Gamma(
                    (self.gnn(data) + torch.tensor(1e-5)) * self.w_sf_mean_var_ratio_tensor,
                    self.w_sf_mean_var_ratio_tensor,
                ),
            )  # (self.n_obs, self.n_factors)
            if (
        ...
        if not self.training_wo_observed:
            # expected expression
            mu = ((a_sf @ self.cell_state) * m_g + (obs2sample @ s_g_gene_add)) * detection_y_s
        ...
        with obs_plate:
            mRNA = a_sf * (self.cell_state * m_g).sum(-1)
    ...
    def compute_expected(self, samples, adata_manager, ind_x=None):
        ...
        mu = (
            np.dot(samples["a_sf"][ind_x, :], self.cell_state_mat.T) * samples["m_g"]
    ...
    def compute_expected_per_cell_type(self, samples, adata_manager, ind_x=None):
        ...
        obs2sample = adata_manager.get_from_registry(REGISTRY_KEYS.BATCH_KEY)
        obs2sample = pd.get_dummies(obs2sample.flatten()).values[ind_x, :]
        mu = np.dot(samples["a_sf"][ind_x, :], self.cell_state_mat.T) * samples["m_g"] + np.dot(
        ...
        mu_ct = [
            csr_matrix(
                x_data.multiply(
                    (
                        np.dot(
                            samples["a_sf"][ind_x, i, np.newaxis],
